...
# 이미지 로드
image = cv2.imread('cave painting.jpg')

cv2.namedWindow('Color Detector')

# Trackbar를 생성하여 'Lower'와 'Upper' 범위 설정
cv2.createTrackbar('Low H', 'Color Detector', 0, 179, nothing)
cv2.createTrackbar('Low S', 'Color Detector', 179, 179, nothing)
cv2.createTrackbar('Low V', 'Color Detector', 0, 255, nothing)
cv2.createTrackbar('High H', 'Color Detector', 255, 255, nothing)
cv2.createTrackbar('High S', 'Color Detector', 0, 255, nothing)
...
while True:
    ...
    high_s = cv2.getTrackbarPos('High H','Color Detector')
    high_v = cv2.getTrackbarPos('High H','Color Detector')
    ...
    if contours:
        ...
        if cv2.contourArea(max_contour) > 1000:
            cv2.drawContours(img, [max_contour], -1, (225,0,0),2)
